...
#setwd('/Users/sarajahfari/Desktop')
...
test=Forestinall[samp[1:floor(0.2*length(samp))],] # define test set (1/3 of trials)
train=Forestinall[-samp[1:floor(0.2*length(samp))],] # define train set (2/3 of trials)
...
sbTrain=as.data.frame(Str_subjects[-sbSamp[1:floor(0.3*length(sbSamp))],])
sbTest=as.data.frame(Str_subjects[sbSamp[1:floor(0.3*length(sbSamp))],])
...
NTREE=2000
# MTRy='SQRT+taskvars'
# lMTRy=floor(sapply(lapply(regions,length),sqrt)+rep(c(1,2,2,3),each=3))
MTRy='lMTRy'
lMTRy=floor(sapply(lapply(regions,length),sqrt)+rep(c(2,3),each=3))
PROX='none'   
...
save(list=c('conf.prox','IMP.prox','AUC.prox','PREDICT.prox'),file=paste('./RF/RF_forests/PAUC_FORESTN43_0.3',
