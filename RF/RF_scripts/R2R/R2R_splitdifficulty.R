...
# cor(Forestinall[Forestinall$Q_Gr_abs=='low',c('Q.diff','Q_abs',"Caudate40exc","Putamen40exc","FFA23")])
# cor(Forestinall[Forestinall$Q_Gr_abs=='med',c('Q.diff','Q_abs',"Caudate40exc","Putamen40exc","FFA23")])
# cor(Forestinall[Forestinall$Q_Gr_abs=='high',c('Q.diff','Q_abs',"Caudate40exc","Putamen40exc","FFA23")])
# 
# cor(Nforestinall[,c('Q.diff','Q_abs',CBase)])
# 
...
# 
# 
# if (Type=='ALL') {Forestinall=Nforestinall[,]} else
#   if (Type=='good') {Forestinall=Nforestinall[Nforestinall$Learner=='good',]} else
#     if (Type=='bad') {Forestinall=Nforestinall[Nforestinall$Learner=='bad',]}
...
# ---------------------- Start looping over vary for Cbase ------------------
...
# hier kan je een for loop bouwen om door de drie categorieeen heen te gaan.
...
for (block in BLOCK)
{
  ...
  ex=2 # Q, response
  IMP.name=matrix(NA,nrow=SEEDS,ncol=length(CBase)-ex) # save variable names of imp sorted
  IMP.Daccuracy=matrix(NA,nrow=SEEDS,ncol=length(CBase)-ex) # save accuracy of imp sorted
  ...
  save(list=c('conf','IMP.name','IMP.Daccuracy','AUC','PREDICT'),
       file=paste('~/Documents/Pearl3T/NEW/RF/RF_forests/splitdiff3_noconf/PAUC_FORESTN43_onetraintest_splitconflict_scaleF','_ntree',NTREE,'_block',block,
                  '_seeds',SEEDS, '_MTRY',MTRy,'type_',Type,'_prox',PROX,'DeltaQ_',DeltaQ,'.Rdat',sep=''))
  
  print(paste('done', block))
}# end block loop

print(paste('done', DeltaQ, Type))
