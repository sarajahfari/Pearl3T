...
CBase=c("response","subject_id",'conflict',
     ...
     "DLPFCposterior","DLPFCanterior","preSMAsmall",'Q.diff')

out='Q.diff'
#out=c("Caudate40exc","Putamen40exc",'Q.diff')
#out=c("Putamen40exc",'Q.diff')
...
for (block in BLOCK)
{
  ...
  # traditional
	...
	save(list=c('conf','IMP.name','IMP.Daccuracy','AUC','PREDICT'),
	    file=paste('~/Documents/Pearl3T/NEW/RF/RF_forests/R2R/PAUC_FORESTN43_onetraintest_DLPFC2','_ntree',NTREE,'_block',block,
